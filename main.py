...
class Grid:
  ...
  def __init__(self, parent):
    ...
    # Sets number of rows and columns in grid to 5.
    self.row = 5
    self.col = 5
    ...
    # Calculate total height canvas based on rows and cell size
    ...
    # Set location of End Goal ("G") cell
    self.grid[0][4] = "G"
    # Set location of Start ("G") cell
    self.grid[4][0] = "S"
    self.rects = []
    ...
    for i in range(self.row):
      ...
      for j in range(self.col):
        ...
        # Calculate the x and y coordinate  for current cell
        x0 = j * self.cell_size
        y0 = i * self.cell_size
        ...
        # Create a rectangle and text object for the current cell
        text = self.canvas.create_text(x0 + self.cell_size // 2,
                                       y0 + self.cell_size // 2,
                                       text=str(self.grid[i][j]))
          
        # Add rectangle and text objects for current cell to row list
  ...
  def congratulate_user(self):
    ...
    # Checks if user got maximum score and congratulate them with message box if they have 
    if list(self.choose_best_route())[0] == self.user_score_maximum():
      if self.click_counter == 9:
        messagebox.showinfo("Maximum score check",
                            "Congratulations! You got the maximum score!")

  def set_start(self, row, column):
    # Sets start button to colour green
    rect, _ = self.rects[row][column]
    self.canvas.itemconfig(rect, fill="green")

  def set_goal(self, row, column):
    # Sets goal button to colour red
    rect, _ = self.rects[row][column]
    self.canvas.itemconfig(rect, fill="red")

  @staticmethod
  def create_path_options():
    # Create all possible combinations of the route by reordering 4 "up"s and 4 "right"s
    root_1 = []
    root_2 = []
    root_3 = []
    root_final = []
  ...
  def show_best_path(self):
    ...
    best_path = list(self.choose_best_route())[1]
    x_coordinate = 30
    y_coordinate = 270
  ...
  def button2Click(self, event):
    ...
    # Reset the canvas with new grid
    for i in range(self.row):
      for j in range(self.col):
    ...
    # Resets S & G to original - doesn't work yet!
    self.set_start(0, 4)
    self.set_goal(4, 0)
    self.grid[0][4] = 0
    self.grid[4][0] = 0
  ...
  # Button 3 exits the program and asks if you really want to quit.
  def on_closing(self):
